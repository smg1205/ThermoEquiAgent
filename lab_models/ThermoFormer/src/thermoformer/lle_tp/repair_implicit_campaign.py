...
from .implicit_roots import final_root_loss
...
def main():
    ...
    p.add_argument('--implicit-every',type=int,default=4)
    p.add_argument('--implicit-start',type=int,default=5)
    p.add_argument('--implicit-samples',type=int,default=4)
    ...
    for seed in args.seeds:
        ...
        for epoch in range(args.epochs):
            model.train();order=np.random.permutation(len(parts['train']));stats=[];accepted_roots=0;implicit_conditions=0
            for start in range(0,len(order),args.batch_size):
                ...
                if epoch>=args.implicit_start and (start//args.batch_size)%args.implicit_every==0 and ternary.any():
                    ids=torch.where(ternary)[0][:args.implicit_samples]
                    root_loss,accepted=final_root_loss(model,select_context(ctx,ids),proposed[ids],target[ids],valid[ids],args.solver_iterations,args.grid_resolution)
                    loss=loss+root_loss
                    accepted_roots+=accepted;implicit_conditions+=len(ids)
            ...
            row={'epoch':epoch,'losses':np.mean(stats,axis=0).tolist(),'seconds':time.time()-started,'implicit_conditions':implicit_conditions,'implicit_successes':accepted_roots}
