...
class BinodalLLE(TPLLE):
    ...
    def context(self,molecules,t,p,mask):
        if not bool((mask.sum(-1)==3).all()):raise ValueError('This decoder currently supports ternary only')
        ...
        return h,m,t,p,q,aa,bb,d,n,pure,mask
    ...
    def propose(self,ctx,points=24):
        ...
        return torch.stack([alpha,beta],2)

    def thermo(self,ctx,x,create_graph=True):
        with torch.enable_grad():
            x=x if x.requires_grad else x.detach().requires_grad_(True)
            qgrid,a,b,d,n,pure=ctx[4:10]
            q=(x*n).sum(-1,keepdim=True);u=(x*d).sum(-1,keepdim=True)
            phi=self.well(qgrid,a,b,q,u)/(x*pure).sum(-1,keepdim=True).clamp_min(1e-20)
            ideal=(x*x.clamp_min(1e-12).log()).sum(-1,keepdim=True)
            affine=(x*(.5*n.square()+2)).sum(-1,keepdim=True)
            gm=.5*q.square()+phi*(2+ideal)-affine
            ge=gm-ideal
            grad=torch.autograd.grad(ge.sum(),x,create_graph=create_graph,retain_graph=True)[0]
            lng=ge+grad-(x*grad).sum(-1,keepdim=True)
            mu=x.clamp_min(1e-12).log()+lng
        return ge,lng,mu,gm
