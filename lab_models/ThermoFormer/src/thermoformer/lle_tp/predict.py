...
def predict(checkpoint,smiles,temperature_k,pressure_kpa,device='cpu',curve_points=24):
    ...
    model=TPLLE(**payload['spec']).to(device).double(); model.load_state_dict(payload['model']); model.eval()
    ...
    with torch.no_grad():
        ...
        proposal=model.propose(ctx,curve_points)[0]
    if n==2: proposal=proposal[:1]
    result=solve(model,ctx,proposal)
