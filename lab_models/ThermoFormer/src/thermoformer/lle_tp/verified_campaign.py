...
from .solver_cached import solve_many
...
def main():
 ...
 conditions,audit=load(args.dataset);scope,mode=args.protocol.split('-');n={'binary':2,'ternary':3,'mixed':None}[scope];conditions=[c for c in conditions if n is None or c.n==n]
 identity={'config':{k:str(v) if isinstance(v,Path) else v for k,v in vars(args).items() if k!='seeds'},'data':audit['inputs'],'source_hashes':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in Path(__file__).parent.glob('*.py')},'model_prior':'binary RK plus continuous parallel ternary tie lines; LLE-only','initialization':'newly initialized network; pretrained molecular features only','recovery_audit_sha256':hashlib.sha256((base.ROOT/'experiments/vle/interpretability/molecular_interactions/figures/lle_tp/source_checks/row_recovery_audit.json').read_bytes()).hexdigest()}
 ...
 for seed in args.seeds:
  base.seed_all(seed);parts=partition(conditions,mode,seed);out=args.output/args.protocol/f'seed_{seed}';out.mkdir(parents=True,exist_ok=True)
