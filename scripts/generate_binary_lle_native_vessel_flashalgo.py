"""Set PropertyPackage.FlashAlgorithm to a NestedLoopsImmiscible instance (the real VLLE route).

The Vessel calculates via PropertyPackage (source line 976: CurrentMaterialStream=MixedStream).
PropertyPackage.FlashAlgorithm (IFlashAlgorithm) is the writable field that selects the
flash kernel.  We construct a NestedLoopsImmiscible and assign it, then run the Vessel.
"""
...
def main():
    ...
    # 1) try to construct NestedLoopsImmiscible and assign to FlashAlgorithm
    ni_type = asm.GetType(
        "DWSIM.Thermodynamics.PropertyPackages.Auxiliary.FlashAlgorithms.NestedLoopsImmiscible"
    )
    print("NestedLoopsImmiscible type:", ni_type is not None and ni_type.FullName)
    if ni_type is not None:
        try:
            inst = Activator.CreateInstance(ni_type)
            inst.StabSearchSeverity = 3
            fa_prop = pp.GetType().GetProperty("FlashAlgorithm")
            print("FlashAlgorithm prop:", fa_prop.PropertyType.FullName, "write=", fa_prop.CanWrite)
            fa_prop.SetValue(pp, inst, None)
            print("[OK] FlashAlgorithm set to NestedLoopsImmiscible")
        except Exception as e:
            print("[WARN] FlashAlgorithm set:", type(e).__name__, e)

    # 2) immiscible-water + 3phase + phase-id switches
    try:
        ...
        print("[OK] FlashSettings switches on")
    except Exception as e:
    ...
    print("\n===== result (FlashAlgorithm=NestedLoopsImmiscible) =====")
